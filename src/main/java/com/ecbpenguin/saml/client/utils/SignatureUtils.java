...
public class SignatureUtils {
	...
	public SignatureUtils(final TinySamlClientConfig config) {
		...
		idpSigningCredential = updateIdpSigningCredential();
	}

	// synchronizing this prevents duplicate updates and minimizes thrashing when the credential changes
	private synchronized Credential updateIdpSigningCredential() {
		...
		return (foundCert != null ) ? new BasicX509Credential(foundCert) : null;
	}
	...
	public boolean validateSignature(final Signature signature) throws SignatureException {
		boolean valid = false;
